...
static void picasso_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void picasso_board_resume(int lp_state, enum resume_stage stg)
{
...
static struct tegra_suspend_platform_data picasso_suspend_data = {
	...
	.corereq_high	= false,
	.sysclkreq_high	= true,
	.board_suspend = picasso_board_suspend,
	.board_resume = picasso_board_resume,
};
...
static struct resource picasso_power_resources[] = {
	[0] = {
		...
		.start = TEGRA_GPIO_TO_IRQ(AC_PRESENT_GPIO),
		.end = TEGRA_GPIO_TO_IRQ(AC_PRESENT_GPIO),
	},
};

static int picasso_is_ac_online(void)
{
	return !gpio_get_value(AC_PRESENT_GPIO);
}

static int picasso_is_usb_online(void)
{
	/* we do not charge over usb */
	return 0;
}

static void picasso_set_charge(int flags)
{
	gpio_direction_output(CHARGING_DISABLE, !flags);
}
...
static int picasso_power_init(struct device *dev)
{
	...
	rc = gpio_request(CHARGING_DISABLE, "Charger Disable");
	if (rc)
		goto err_chg;

	pr_info("gpio charger disable\n");

	rc = gpio_request(AC_PRESENT_GPIO, "Charger Detection");
	if (rc)
		goto err_ac;

	pr_info("gpio charger detection\n");

	return 0;

err_ac:
	gpio_free(CHARGING_DISABLE);
err_chg:
	return rc;
}
...
static struct platform_device picasso_charger_device = {
	.name = "pda-power",
	.id = -1,
...
/* todo: fix charger ( gpio ) */
int __init picasso_charger_init(void)
{
	pr_info("%s\n", __FUNCTION__);

	platform_device_register(&picasso_charger_device);
